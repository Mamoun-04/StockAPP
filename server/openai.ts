  ...
  app.post("/api/ai/lesson", async (req, res) => {
    try {
        ...
        messages: [
          {
            ...
            The content should be structured as a series of sections, each with detailed content.
      ...
      const [newLesson] = await db.insert(lessons).values({
        ...
        content: JSON.stringify(lessonContent.sections),
        difficulty: difficulty,
        xpReward: lessonContent.sections.reduce((sum: number, section: any) => sum + section.xpReward, 0),
        order: 1, // You might want to adjust this based on your needs
      }).returning();

      // Store the flashcards
      if (lessonContent.flashcards && lessonContent.flashcards.length > 0) {
        await db.insert(flashcards).values(
          lessonContent.flashcards.map((card: any, index: number) => ({
